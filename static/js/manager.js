...
function switchMainTab(tab) {
    ...
    ['viewStats', 'viewPending', 'viewEdit', 'viewChargebacks', 'viewAnalysis'].forEach(id => {
...
function renderAnalysis() {
    ...
    const getStatus = (r) => {
        const k = Object.keys(r).find(key => key.toLowerCase().includes('status'));
        return k ? r[k] : 'Unknown';
    };
            ...
            <div class="bg-slate-800 p-4 rounded-lg border border-slate-700 flex justify-between items-center group hover:border-red-500/50 transition">
                <div>
                    <div class="font-bold text-white flex items-center gap-2">
                        ${agent} 
                        <span class="bg-slate-700 text-slate-300 text-[10px] px-1.5 py-0.5 rounded">#${id}</span>
                    </div>
                ...
                <button onclick="markAsChargeback('${sheet}', '${id}')" class="bg-red-900/30 hover:bg-red-600 text-red-200 hover:text-white px-4 py-2 rounded-lg font-bold transition border border-red-800/50">
                    Mark Chargeback
                </button>
            </div>
        `;
