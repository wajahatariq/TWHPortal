...
/* =========================================
   COPY & PASTE AT THE END OF manager.js
   FEATURE: "Casino Slot Machine" Number Counters
   ========================================= */
(function() {
    // Configuration
    const ANIMATION_DURATION = 2000; // 2 seconds spin

    // 1. The Rolling Function
    function animateValue(obj, start, end) {
        ...
        const step = (timestamp) => {
            ...
            // Ease-out expo function for the "landing" feel
    ...
    // 2. Observer (Watches for changes in the data)
    // This connects to your existing 'allData' variable logic
    let cache = {};

    setInterval(() => {
        // Find all elements that look like money (IDs starting with 'stats' or containing 'total')
        const moneyElements = document.querySelectorAll('[id*="total"], [id*="stats"]');
        
        moneyElements.forEach(el => {
            // Get raw number
            const rawTxt = el.innerText.replace(/[^0-9.-]+/g,"");
            const val = parseFloat(rawTxt);
            
            if (!isNaN(val)) {
                const id = el.id || Math.random().toString(); // Fallback ID
                
                // If value changed from last check
                if (cache[id] !== undefined && cache[id] !== val) {
                    // Play Slot Machine Animation
                    animateValue(el, cache[id], val);
                    
                    // Optional: Play a faint "tick" sound?
                    // new Audio('/static/sounds/tick.mp3').play();
                }
                
                cache[id] = val;
            }
        });
    }, 2000); // Check every 2 seconds matching your polling
})();
