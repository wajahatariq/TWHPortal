let nightStats = { telecom: {total:0, breakdown:{}}, billing: {total:0, breakdown:{}} };
...
function updateNightWidget() {
    ...
    if (data.breakdown && Object.keys(data.breakdown).length > 0) {
        ...
        sortedEntries.forEach(([agent, amount]) => {
...
function toggleProviderFields() {
    ...
    else if (provider === 'Optimum') accDiv.classList.remove('hidden');
}
...
function clearForm() {
    const form = document.getElementById('telecomForm');
    ...
    submitBtn.innerText = "Submit Telecom";
    submitBtn.classList.replace('bg-green-600', 'bg-cyan-600');
...
async function searchLead(specificRowIndex = null) {
    ...
    let url = `/api/get-lead?type=telecom&id=${id}`;
    ...
    try {
        ...
        if(json.status === 'multiple') {
            ...
            json.data.forEach(c => {
                ...
                item.className = "p-3 bg-slate-700/50 rounded-lg cursor-pointer hover:bg-cyan-600/50 border border-slate-600 transition flex justify-between items-center mb-2";
        ...
        if(json.status === 'success') {
            ...
            submitBtn.classList.replace('bg-cyan-600', 'bg-green-600');
    ...
    } catch(e) { showToast("Error fetching data", true); } 
    finally { if(!specificRowIndex && btn) btn.innerText = "Find"; }
}
...
if(newLeadBtn) {
    newLeadBtn.addEventListener('click', async function() {
        ...
        formData.set('row_index', '');
        formData.set('original_timestamp', '');
        ...
        finally { newLeadBtn.innerText = originalText; newLeadBtn.disabled = false; }
    });
}
