...
/* =========================================
   ...
   "The Wall of Shame" (Top Left Roast)
   ========================================= */
(function() {
    // 1. Roast Menu
    const shameRoasts = [
        "Is your monitor even on? 🔌",
        "Emotional support hire. ❤️",
        "I've seen glaciers move faster. 🧊",
        "Are you waiting for a written invitation? ✉️",
        "Loading motivation... 0% completed. ⏳",
        "At least you're pretty? 🤷‍♂️",
        "Giving everyone else a head start? 🏁",
        "Hello? Is it me you're looking for? 📞",
        "Purely decorative at this point. 🪴",
        "Trying to set a low score record? 📉"
    ];

    // 2. Function to Update the Wall of Shame
    function updateWallOfShame() {
        // Access the global nightStats variable (from your existing code)
        // Default to 'billing' or whatever is selected in the main widget
        ...
        const shameDiv = document.getElementById('wallOfShame');
        const agentDiv = document.getElementById('shameAgent');
        const quoteDiv = document.getElementById('shameQuote');

        if (!data.breakdown || Object.keys(data.breakdown).length === 0) {
            // Hide if no data yet
            if(shameDiv) shameDiv.classList.add('hidden');
            return;
        }

        // Show widget
        if(shameDiv) shameDiv.classList.remove('hidden');

        // 3. Find the Lowest Performer
        // Convert object to array: [['AgentName', 100], ['Agent2', 50]]
        ...
        if(agentDiv) {
            agentDiv.innerHTML = `
                <div>${loserName}</div>
                <div class="text-sm text-slate-500 font-mono">$${loserAmount.toFixed(2)}</div>
            `;
        }

        // 5. Update Roast (Only change roast if the loser changes to avoid flickering)
        if(quoteDiv && shameDiv.dataset.lastLoser !== loserName) {
            const randomRoast = shameRoasts[Math.floor(Math.random() * shameRoasts.length)];
            quoteDiv.innerText = `"${randomRoast}"`;
            shameDiv.dataset.lastLoser = loserName;
        }
    }

    // 3. Run this check periodically (every 2 seconds)
    // We do this to ensure it stays in sync with your main fetch loop
    setInterval(updateWallOfShame, 2000);
})();


/* =========================================
   COPY & PASTE THIS AT THE END OF billing.js
   "The Reality Check" (Lifestyle Tracker)
   ========================================= */
(function() {
    // 1. Session Storage
    let sessionTotal = parseFloat(sessionStorage.getItem('mySessionTotal')) || 0;
    
    // 2. The Menu (Price Points)
    const lifestyleMenu = [
        { price: 0,    item: "Tap Water 🚰", quote: "It's free and has minerals." },
        { price: 50,   item: "Instant Noodles 🍜", quote: "Chicken flavor. Luxury." },
        { price: 150,  item: "Gas Money ⛽", quote: "Now you can actually go home." },
        { price: 300,  item: "Netflix (With Ads) 📺", quote: "Don't get too crazy now." },
        { price: 500,  item: "Used Honda Civic Tire 🚗", quote: "Just one. Keep working." },
        { price: 800,  item: "Guacamole at Chipotle 🥑", quote: "Look at Mr. Moneybags here." },
        { price: 1200, item: "Knockoff Rolex ⌚", quote: "It ticks loudly, but it shines." },
        { price: 2000, item: "Vegas Weekend (Budget) 🎰", quote: "Don't bet it all on Red." },
        { price: 5000, item: "Your Boss's Respect 🤝", quote: "Just kidding, keep dialing." }
    ];

    function updateRealityCheck() {
        const itemEl = document.getElementById('lifestyleItem');
        const quoteEl = document.getElementById('lifestyleQuote');
        const barEl = document.getElementById('lifestyleBar');
        const nextEl = document.getElementById('nextLifestyleCost');
        
        if(!itemEl) return;

        // Find current lifestyle
        let current = lifestyleMenu[0];
        let next = lifestyleMenu[1];

        for (let i = 0; i < lifestyleMenu.length; i++) {
            if (sessionTotal >= lifestyleMenu[i].price) {
                current = lifestyleMenu[i];
                next = lifestyleMenu[i+1] || null;
            }
        }

        // Render
        itemEl.innerText = current.item;
        quoteEl.innerText = `"${current.quote}"`;

        if (next) {
            nextEl.innerText = `$${next.price}`;
            // Calculate progress to next item
            // (Current Total - Current Item Price) / (Next Item Price - Current Item Price)
            const progress = (sessionTotal - current.price) / (next.price - current.price);
            barEl.style.width = `${Math.max(0, Math.min(100, progress * 100))}%`;
        } else {
            nextEl.innerText = "MAX LEVEL";
            barEl.style.width = "100%";
        }
        
        sessionStorage.setItem('mySessionTotal', sessionTotal);
    }

    // 3. Hook into Toast Logic (Same as before)
    const existingToastForReality = window.showToast;
    window.showToast = function(msg, isError = false) {
        if(!isError && (msg.toLowerCase().includes('saved') || msg.toLowerCase().includes('success'))) {
            const chargeInput = document.getElementById('charge_amt');
            if(chargeInput) {
        ...
        if(existingToastForReality) existingToastForReality(msg, isError);
    };
